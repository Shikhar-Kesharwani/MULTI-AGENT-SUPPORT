...
```bash
cp infra/docker/.env.example infra/docker/.env # fill in production values
...
│ (postgres-data vol│   │ (redis-data vol)  │   │ (shares backend   │
│                   │   │                   │   │   code)           │
└───────────────────┘   └───────────────────┘   └───────────────────┘
```
All containers on a single user-defined bridge network ("app-network"). Only Nginx publishes ports to the host.
